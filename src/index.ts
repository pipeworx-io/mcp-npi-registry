...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'NPI Registry');
}
...
async function search(args: Record<string, unknown>) {
  ...
  if (!hasFilter) {
    throw new Error('user_error: NPI Registry requires at least one filter (number, name, organization_name, postal_code, state, etc).');
  }
